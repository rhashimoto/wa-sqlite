...
export function api_statements(context) {
  describe('statements', function() {
    ...
    it('should read text with NUL bytes', async function() {
      // From #331. The leading BOM is part of the case: a decoder that strips
      // one would eat it here.
      let rc;
    ...
    it('should read text that is only a BOM', async function() {
      let rc;
      const sql = 'SELECT char(65279)';
    ...
    it('should return null for a NULL text column', async function() {
      let rc;
      ...
      for await (const stmt of i(sqlite3.statements(db, sql))) {
        while ((rc = await sqlite3.step(stmt)) !== SQLite.SQLITE_DONE) {
          expect(rc).toEqual(SQLite.SQLITE_ROW);
          expect(await sqlite3.column_text(stmt, 0)).toBeNull();
        }
      }
    });
